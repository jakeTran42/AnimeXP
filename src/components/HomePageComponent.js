...
class HomePageComponent extends Component {
  ...
  render() {
    ////console.log("This is HomePageComponent state:", this.state)
    ////console.log("This is HomePageComponent props:", this.props)
    // Though it says to make API calls in `componentDidMount`, doing it here for speed
    this.setupNetworkCalls();
    return (
      <div style={styles['container']}>
        {this.loadModals()}
        <Container style={{"margin": 0, "padding": 0, "max-width": "100%", height: '100vh','overflow': 'scroll', '-webkit-overflow-scrolling': 'touch',}}>

          <Row style={{...styles['section-horizontal-bottom'], ...styles['anime-horizontal-scroll-row'], "margin": 0}}>
            <span style={{...styles['anime-list-title-section'], 'font-size': '2em', 'margin-top': '10px'}}>Most Popular</span>
            <div style={{...styles['container-for-horizontal-scroll-container'], "width": "100vw", 'height': '300px'}}>
              <section style={{'overflow-y': 'visible', '-webkit-overflow-scrolling': 'touch', ...styles['horizontal-scroll-container-main']}}>
                {this.loadMostPopularAnime()}
              </section>
            </div>
            <LoadingSpinnerComponent isLoading={this.props.mostPopularAnime ? false : true} style={{'margin-top': '-300px'}} multiplier={3}/>
          </Row>
          {this.loadLowerSection()}
        </Container>
      </div>
